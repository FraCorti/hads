...
class Reds_BatchNormalizationBase(tf.keras.layers.Layer):
    ...
    *after having been trained on data that has similar statistics as the
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        inputs_dtype = inputs[0].dtype.base_dtype
        ...
        mean = tf.cast(mean, inputs[0].dtype)
        variance = tf.cast(variance, inputs[0].dtype)
        if offset is not None:
            offset = tf.cast(offset, inputs[0].dtype)
        if scale is not None:
            scale = tf.cast(scale, inputs[0].dtype)

        for subnet_number in range(len(inputs)):

            input_dimensions = inputs[subnet_number].shape.dims[-1].value

            inputs[subnet_number] = tf.nn.batch_normalization(
                x=inputs[subnet_number],
                mean=mean[0:input_dimensions],
                variance=variance[0:input_dimensions],
                offset=offset[0:input_dimensions],
                scale=scale[0:input_dimensions],
                variance_epsilon=self.epsilon,
            )
            if inputs_dtype in (tf.float16, tf.bfloat16):
                inputs[subnet_number] = tf.cast(inputs[subnet_number], inputs_dtype)
