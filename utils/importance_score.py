...
def compute_accumulated_gradients(model, train_data, loss_fn, args):
    """
    Accumulate a number of gradients equal to the number of minibatches defined
    @param model: pretrained model
    @param train_data: training dataset
...
def assign_pretrained_ds_convolution_filters(model, permuted_convolutional_filters, permuted_convolutional_bias,
...
def compute_pointwise_importance_score_ds_cnn(model, gradients_accumulation_pointwise, int_scale=1e5):
    """
            ...
            """
    model_pointwise_layers_weights, model_pointwise_layers_biases = get_pointwise_convolutions_layers_weights(
        model=model)
