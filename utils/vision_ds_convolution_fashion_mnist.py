...
from utils.datasets import get_dataset, get_cifar10_train_test, get_fashion_mnist_train_test
...
if __name__ == '__main__':
    ...
    for model_size in args.model_sizes.split(','):
        ...
        for experimental_run in range(args.experimental_runs):
            ...
            lr_schedule_head_finetuning = tf.keras.optimizers.schedules.PiecewiseConstantDecay(
                boundaries=[16406, 30468],
            ...
            train_data, test_data = get_fashion_mnist_train_test(batch_size=128)
            ...
            optimizer_permute_model = tf.keras.optimizers.experimental.Adam()  # learning_rate=lr_schedule_head_finetuning
            ...
            pointwise_filters_batch_norm_finetuned_accuracy = classification_head_finetuning_ds_cnn(
                ...
                initial_pretrained_test_accuracy=90.5, print_info=False)
            ...
            print("Time taken to finetune and permute the model based on importance score: ", end - start)
            ...
            optimizer = tf.optimizers.experimental.Adam(learning_rate=lr_schedule_head_finetuning)
            ...
            optimizer_batch = tf.keras.optimizers.experimental.Adam(learning_rate=lr_schedule_head_finetuning)
